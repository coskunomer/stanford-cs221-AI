...
def formula2d():
    ...
    return Forall("$x", Forall("$z", Equiv(And(Exists("$y", And(Parent("$x", "$y"), Parent("$y", "$z"))), Female("$z")), Grandmother("$x", "$z"))))
    # END_YOUR_CODE
...
def liar():
    ...
    formulas.append(Exists("$x", Forall("$y", Equiv(And(TellTruth("$x"), Not(Equals("$x", "$y"))), Not(TellTruth("$y"))))))
    formulas.append(Exists("$x", Forall("$y", Equiv(And(CrashedServer("$x"), Not(Equals("$x", "$y"))), Not(CrashedServer("$y"))))))
    # END_YOUR_CODE
...
def ints():
    ...
    formulas.append(Forall("$x", Exists("$y", And(Forall("$z", Equiv(Successor("$x", "$z"), Equals("$y", "$z"))), Not(Equals("$x", "$y"))))))
    formulas.append(Forall("$x", Equiv(Even("$x"), Not(Odd("$x")))))
...
def createRule1():
    ...
    #return GrammarRule('$Clause', ['every', '$Noun', '$Verb', 'some', '$Noun'], lambda args: Forall("$x", And(Atom(args[0].title(), '$x'), Exists('$y', And(Atom(args[2].title(), '$y'), Atom(args[1].title(), args[0].lower(), '$y'))))))
    # END_YOUR_CODE

def createRule2():
    # Return a GrammarRule for 'there is some $Noun that every $Noun $Verb'
    # BEGIN_YOUR_CODE (our solution is 3 lines of code, but don't worry if you deviate from this)
    #return GrammarRule('$Clause', ['there', 'is', 'some', '$Noun', 'that', 'every', '$Noun', '$Verb'], lambda args:
    #                   Exists("$x", And(Atom(args[0].title(), "$x"), Forall("$y", And(Atom(args[1].title(), "$y"), Atom(args[2].title(), args[1].lower(), "$x"))))))
    # END_YOUR_CODE

def createRule3():
    # Return a GrammarRule for 'if a $Noun $Verb a $Noun then the former $Verb the latter'
    # BEGIN_YOUR_CODE (our solution is 4 lines of code, but don't worry if you deviate from this)
    #return GrammarRule('$Clause', ['if', 'a', '$Noun', '$Verb', 'a', '$Noun', 'then', 'the', 'former', '$Verb', 'the', 'latter'], lambda args:
    #                   Implies(Atom(args[1].title(), args[0].lower(), args[2].lower()), Atom(args[3].title(), args[0].lower(), args[2].lower())))
    # END_YOUR_CODE
