...
def create_chain_csp(n):
    ...
    for i in range(n-1):
        csp.add_binary_factor(variables[i], variables[i+1], lambda var1, var2: var1 != var2)
    # END_YOUR_CODE
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    variables = ["queen" + str(i) for i in range(1, n+1)]
    ...
    for i in range(n-1):
        for j in range(i+1, n):
            x = variables[i]
            y = variables[j]
            csp.add_binary_factor(x, y, lambda x, y: x != y and x + (j-i) != y and x + (i-j) != y)
    # END_YOUR_CODE
    return csp
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_CODE (our solution is 7 lines of code, but don't worry if you deviate from this)
            most_constrained_so_far = [None, float("inf")]
            for var in self.csp.variables:
                if var not in assignment:
                    ...
                    for val in self.domains[var]:
                        if self.get_delta_weight(assignment, var, val) > 0:
                            domain += 1
                    if domain < most_constrained_so_far[1]:
                        most_constrained_so_far[0] = var
                        most_constrained_so_far[1] = domain
            return most_constrained_so_far[0]
            # END_YOUR_CODE
    ...
    def arc_consistency_check(self, var):
        ...
        neighbors = [(var2, var) for var2 in self.csp.get_neighbor_vars(var)]
        while len(neighbors) != 0:
            var2, var1 = neighbors.pop(0)
            remove_list = []
            for val2 in self.domains[var2]:
                changed = True
                for val1 in self.domains[var1]:
                    if self.csp.binaryFactors[var2][var1][val2][val1] > 0:
                        changed = False
                if changed: remove_list.append(val2)
            if remove_list:
                for item in remove_list:
                    self.domains[var2].remove(item)
                for var3 in self.csp.get_neighbor_vars(var2):
                    if var3 != var2:
                        neighbors.append((var3, var2))
        # END_YOUR_CODE
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    if variables:
        ...
        for i, var in enumerate(variables):
            A_i = ("sum", name, i)
            if i == 0:
                csp.add_variable(A_i, [(0, i) for i in range(0, maxSum+1)])
                csp.add_binary_factor(A_i, var, lambda A_i, var: A_i[1] == var)
            else:
                csp.add_variable(A_i, [(i, j) for i in range(0, maxSum+1) for j in range(0, maxSum+1)])
                csp.add_binary_factor(A_i, var, lambda A_i, var: A_i[1] == var)
                csp.add_binary_factor(A_i, previous, lambda A_i, previous: A_i[0] == sum(previous))
            previous = A_i
        csp.add_binary_factor(result, A_i, lambda result, A_i: result == sum(A_i))
    else:
        result = ("sum", name, "aggregated")
...
class SchedulingCSPConstructor():
    ...
    def add_quarter_constraints(self, csp):
        ...
        for request in self.profile.requests:
            for quarter in self.profile.quarters:
                csp.add_unary_factor((request, quarter), lambda cid: cid is None or (self.bulletin.courses[cid].is_offered_in(quarter) and quarter in request.quarters))
        # END_YOUR_CODE
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                for cid in request.cids:
                    ...
                    if course.is_offered_in(quarter) and course.minUnits <= self.profile.maxUnits:
                        csp.add_variable((cid, quarter), [0] + list(range(course.minUnits, course.maxUnits+1)))
                        csp.add_binary_factor((cid, quarter), (request, quarter), lambda unit, req: unit != 0 if req == cid else unit==0 )
                        courses.append((cid, quarter))
                        maxSum += course.maxUnits
            maxSum = min(self.profile.maxUnits, maxSum)
            name = "sum-of-units-{}".format(quarter)
            result = get_sum_variable(csp, name, courses, maxSum)
            csp.add_unary_factor(result, lambda result: result >= self.profile.minUnits and result <= self.profile.maxUnits)
        # END_YOUR_CODE
