...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                y2 = util.rowToY(row)
                x2 = util.colToX(col)
                distance = ((y2 - agentY)**2 + (x2 - agentX)**2)**0.5
                prob = util.pdf(distance, Const.SONAR_STD, observedDist) * self.belief.getProb(row, col)
    ...
    def elapseTime(self):
        ...
        for oldTile, newTile in self.transProb:
            prob = self.belief.getProb(oldTile[0], oldTile[1]) * self.transProb.get((oldTile, newTile), 0)
            belief.addProb(newTile[0], newTile[1], prob)
        belief.normalize()
        self.belief = belief
        # END_YOUR_CODE
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        weightsDict = collections.defaultdict(float)
        newParticles = collections.defaultdict(int)
        for tile in self.particles:
            x_tile = util.colToX(tile[1])
            y_tile = util.rowToY(tile[0])
            distance = ((agentX-x_tile)**2 + (agentY-y_tile)**2)**0.5
            particles = util.pdf(distance, Const.SONAR_STD, observedDist) * self.particles[tile]
            weightsDict[tile] = particles
        for tile in range(self.NUM_PARTICLES):
            newParticles[util.weightedRandomChoice(weightsDict)] += 1
